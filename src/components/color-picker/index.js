...
module.exports = class ColorPicker extends Component {
  ...
  updateValue = ({ nativeEvent }, val) => {
    ...
    const v = typeof val === "number" ? val : 100 * this.ratio(nativeEvent)
    const hsv = { h, s, v }
    const currentColor = hsv2Hex(hsv)
    this.color = hsv
    this.setState({ hsv, currentColor, hueSaturation: hsv2Hex(this.color.h, this.color.s, 100) })
  ...
  render() {
    ...
    const hex = hsv2Hex(this.color.h, this.color.s, 100)
      ...
      <View style={[ss.root, row ? { flexDirection: "row" } : {}, style]}>
        ...
        {!swatchesOnly && (
          <View style={ss.wheel} key={"$1"} onLayout={this.onSquareLayout}>
            {this.wheelWidth > 0 && (
              <View
                style={{ padding: thumbSize / 2, width: this.wheelWidth, height: this.wheelWidth }}
              >
